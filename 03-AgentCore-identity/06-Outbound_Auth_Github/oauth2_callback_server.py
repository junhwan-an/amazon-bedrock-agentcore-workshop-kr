...
# OAuth2 callback server의 설정 상수
OAUTH2_CALLBACK_SERVER_PORT = 9090  # callback server가 수신 대기하는 포트
PING_ENDPOINT = "/ping"  # 헬스 체크 엔드포인트
OAUTH2_CALLBACK_ENDPOINT = (
    "/oauth2/callback"  # provider 리다이렉트를 위한 OAuth2 callback 엔드포인트
)
...
class OAuth2CallbackServer:
    ...
    def __init__(self, region: str):
        ...
        """
        # 지정된 region에 대한 AgentCore Identity client 초기화
        self.identity_client = IdentityClient(region=region)

        # 사용자 토큰 식별자 저장소 - OAuth 세션을 특정 사용자에게 바인딩하는 데 사용
    ...
    def _setup_routes(self):
        ...
        async def _handle_oauth2_callback(session_id: str):
            ...
            # OAuth 세션을 올바른 사용자에게 바인딩하기 위해 필요함
            ...
            # 이는 OAuth 세션을 사용자와 연결하고 액세스 토큰을 검색함
...
def wait_for_oauth2_server_to_be_ready(
    ...
    while time.time() - start_time < timeout_in_seconds:
        ...
        # 여전히 대기 중임을 표시하기 위해 10초마다 진행 상황 로그
...
def main():
    ...
    # 환경에 따라 호스트 바인딩 결정
    # SageMaker에서는 proxy가 서버에 도달할 수 있도록 0.0.0.0에 바인딩
