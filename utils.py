...
def setup_cognito_user_pool(pool_name="MCPServerPool"):
    ...
    try:
        # User Pool 생성 (최소 비밀번호 길이 8자)
        user_pool_response = cognito_client.create_user_pool(
            PoolName=pool_name, Policies={"PasswordPolicy": {"MinimumLength": 8}}
        )
        pool_id = user_pool_response["UserPool"]["Id"]
        
        # App Client 생성 (비밀키 없이 사용자명/비밀번호 인증 허용)
        app_client_response = cognito_client.create_user_pool_client(
            UserPoolId=pool_id,
            ClientName="MCPServerPoolClient",
            GenerateSecret=False,  # 비밀키 생성 안 함
        ...
        cognito_client.admin_create_user(
            ...
            MessageAction="SUPPRESS",  # 환영 이메일 발송 안 함
        )
...
def reauthenticate_user(client_id):
    ...
    # 기존 사용자로 재인증하여 새로운 Access Token 발급
    auth_response = cognito_client.initiate_auth(
        ClientId=client_id,
        AuthFlow="USER_PASSWORD_AUTH",
        AuthParameters={"USERNAME": USER_NAME, "PASSWORD": PASSWORD},
...
def create_agentcore_role(agent_name):
    ...
    # bedrock-agentcore 서비스가 이 Role을 사용할 수 있도록 신뢰 정책 정의
