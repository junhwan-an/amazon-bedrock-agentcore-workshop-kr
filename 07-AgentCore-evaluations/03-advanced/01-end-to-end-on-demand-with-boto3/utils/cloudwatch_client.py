...
class CloudWatchQueryBuilder:
    ...
    def build_spans_by_session_query(session_id: str, agent_id: str = None) -> str:
        ...
        if agent_id:
            # resource_id에서 agent ID 추출 후 필터링 (예: "runtime/agent-123/..." -> "agent-123")
    ...
    def build_runtime_logs_by_traces_batch(trace_ids: List[str]) -> str:
        ...
        # CloudWatch Logs Insights의 'in' 연산자를 위해 ['id1', 'id2'] 형식으로 변환
...
class ObservabilityClient:
    ...
    def query_spans_by_session(
        ...
        # CloudWatch 쿼리 결과의 각 row를 Span 객체로 변환
    ...
    def _execute_cloudwatch_query(
        ...
        try:
            # CloudWatch Logs Insights 쿼리 시작 (비동기 실행)
            response = self.logs_client.start_query(
                logGroupName=log_group_name,
                startTime=start_time // 1000,  # 밀리초를 초 단위로 변환
