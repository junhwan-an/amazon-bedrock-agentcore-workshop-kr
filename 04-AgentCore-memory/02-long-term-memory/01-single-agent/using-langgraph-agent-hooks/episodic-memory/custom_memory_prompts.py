# LLM에게 대화에서 에피소드 기억을 추출하도록 지시하는 프롬프트
# 영양 및 식사 계획 관련 대화에서 의미있는 에피소드를 추출
...
# 새로운 에피소드 기억을 기존 기억과 비교하여 처리 방법을 결정하는 프롬프트
# AddMemory(추가), UpdateMemory(업데이트), SkipMemory(건너뛰기) 중 하나를 선택
