...
def setup_cognito_user_pool():
    ...
    try:
        ...
        # App Client 생성 (GenerateSecret=False: client secret 없이 public client로 생성)
        app_client_response = cognito_client.create_user_pool_client(
            UserPoolId=pool_id,
            ClientName='MCPServerPoolClient',
            GenerateSecret=False,
            ExplicitAuthFlows=[
                'ALLOW_USER_PASSWORD_AUTH',  # username/password 직접 인증 허용
                'ALLOW_REFRESH_TOKEN_AUTH'   # refresh token을 통한 재인증 허용
            ]
        )
        client_id = app_client_response['UserPoolClient']['ClientId']
        
        # 사용자 생성 (MessageAction='SUPPRESS': 이메일/SMS 알림 비활성화)
        ...
        # 영구 비밀번호 설정 (Permanent=True: 비밀번호 변경 강제하지 않음)
...
def get_or_create_user_pool(cognito, USER_POOL_NAME):
    ...
    for pool in response["UserPools"]:
        if pool["Name"] == USER_POOL_NAME:
            ...
            # user pool 설명에서 domain 가져오기
            user_pool = response.get('UserPool', {})
            domain = user_pool.get('Domain')
        
            if domain:
                # user_pool_id 형식: us-east-1_xxxxxxxxx에서 region 추출
    ...
    # domain 이름은 소문자와 하이픈만 허용되므로 underscore 제거 후 소문자 변환
...
def get_or_create_m2m_client(cognito, user_pool_id, CLIENT_NAME, RESOURCE_SERVER_ID, SCOPES=None):
    ...
    # 제공되지 않은 경우 기본 scope (하위 호환성을 위해)
    if SCOPES is None:
        SCOPES = [f"{RESOURCE_SERVER_ID}/gateway:read", f"{RESOURCE_SERVER_ID}/gateway:write"]

    # Machine-to-Machine (M2M) client 생성
    # - GenerateSecret=True: client secret 생성 (서버 간 통신용)
    # - AllowedOAuthFlows=["client_credentials"]: OAuth 2.0 client credentials grant 사용
    # - AllowedOAuthFlowsUserPoolClient=True: OAuth flow 활성화
...
def get_token(user_pool_id: str, client_id: str, client_secret: str, scope_string: str, REGION: str) -> dict:
    """OAuth 2.0 client credentials grant를 사용하여 access token 획득"""
    try:
        # Cognito OAuth 2.0 token endpoint URL 구성
        ...
        data = {
            "grant_type": "client_credentials",  # M2M 인증 방식
            "client_id": client_id,
...
def create_agentcore_role(agent_name):
    """Bedrock AgentCore runtime을 위한 IAM role 생성"""
    ...
    role_policy = {
        ...
        "Statement": [
            ...
            "Effect": "Allow",
            "Action": [
                "xray:PutTraceSegments",  # X-Ray 분산 추적
             ...
             },
             {
                ...
                "Condition": {
                    "StringEquals": {
                        "cloudwatch:namespace": "bedrock-agentcore"  # 특정 namespace로 제한
                    }
                }
            },
             {
    ...
    # Trust policy: bedrock-agentcore 서비스가 이 role을 assume할 수 있도록 허용
        ...
        "Statement": [
            {
                ...
                "Condition": {
                    "StringEquals": {
                        "aws:SourceAccount": f"{account_id}"  # 동일 계정에서만 assume 가능
                    },
                    "ArnLike": {
    ...
    try:
        ...
        # role이 생성되었는지 확인하기 위해 일시 중지 (IAM 전파 대기)
        time.sleep(10)
...
def create_gateway_lambda(lambda_function_code_path) -> dict[str, int]:
    """Gateway용 Lambda 함수 생성 (IAM role 포함)"""
    boto_session = Session()
    region = boto_session.region_name
    ...
    try:
        ...
        # Lambda 기본 실행 권한 (CloudWatch Logs 작성 등)
        ...
        # IAM role 전파 대기 (일반적으로 10초면 충분하지만 여기서는 100초 대기)
        time.sleep(100)
    ...
    if role_arn != "":
        ...
        try:
            lambda_response = lambda_client.create_function(
                ...
                Role=role_arn,
                Runtime='python3.12',
                Handler='lambda_function_code.lambda_handler',  # zip 내부 파일명.함수명
...
def create_gateway_invoke_tool_role(role_name, gateway_id, current_arn):
    """Gateway 호출을 위한 IAM role 생성 (양방향 trust 설정)"""
    # current_arn 정규화 (set/list/tuple을 string으로 변환)
    ...
    # AgentCore와 현재 caller 모두 이 role을 assume할 수 있도록 설정
    assume_role_policy_document = {
        "Version": "2012-10-17",
        "Statement": [
            {
    ...
    # ARN 형식: arn:aws:iam::123456789012:user/username 또는 role/rolename
...
def grant_gateway_invoke_permission(function_name, region='us-east-1'):
    ...
    try:
        # Lambda resource-based policy에 bedrock-agentcore 서비스 호출 권한 추가
...
def delete_iam_role(role_name):
    ...
    try:
        # 관리형 정책 분리 (AWS managed 또는 customer managed policies)
        ...
        # 인라인 정책 삭제 (role에 직접 embedded된 정책)
